...
class _WLS:
    """
    加权最小二乘，支持三种权重估计方法：
      weight_method="park"    — Park Test：辅助回归估计异方差，适合截面/日线
      weight_method="rolling" — 滚动窗口标准差：捕捉时变波动率，适合分钟线时序
      weight_method="ewma"    — EWMA标准差估计：近期残差权重高，远期自然衰减

    改进项（相对原版）：
      1. 前视偏差修复   — 残差由 expanding OLS 计算，第 t 天只用 [0,t) 的数据
      2. 量纲统一       — 三种方法统一使用 w = 1/σ（标准差倒数）而非 1/σ²
      3. 权重截断       — Winsorize [1%, 99%]，防止极端低波动期的权重爆炸
      4. 公共残差方法   — _expanding_resid() 被三种权重方法共用，逻辑一致
    """
    ...
    def __init__(self,
        ...
        self.min_fit_samples  = min_fit_samples   # expanding OLS 最少样本数
        self.winsorize        = winsorize          # 是否截断极端权重
        self.winsorize_bounds = winsorize_bounds   # 截断分位数
        self.coef_            = None
        self.intercept_       = 0.0

    # ══════════════════════════════════════════════════════════════
    # 公共工具：expanding OLS 残差（消除前视偏差）
    # ══════════════════════════════════════════════════════════════
    def _expanding_resid(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        """
        第 t 天的残差只用 [0, t) 的数据拟合的系数来预测，彻底消除前视偏差。

        前 min_fit_samples 天因历史不足，退化为全样本 OLS 残差填充
        （这部分有偏，但占比小且无法完全避免）。

        时间复杂度：O(n² × p)，n=1300、p=10 时约 0.5 秒，可接受。
        """
        n = len(y)
        resid = np.zeros(n)

        # 最少需要 max(p+2, min_fit_samples) 个样本才能稳定拟合
        min_fit = max(X.shape[1] + 2, self.min_fit_samples)

        # ── 有足够历史的部分：expanding OLS ──────────────────────
        for t in range(min_fit, n):
            m = LinearRegression().fit(X[:t], y[:t])
            resid[t] = y[t] - m.predict(X[t:t+1])[0]

        # ── 头部不足的部分：全样本 OLS 兜底 ─────────────────────
        # 已知有偏，但前 min_fit 天权重在后续回归中占比极小
    ...
    # ══════════════════════════════════════════════════════════════
    # 公共工具：权重后处理（Winsorize + 归一化）
    # ══════════════════════════════════════════════════════════════
    def _postprocess(self, w: np.ndarray) -> np.ndarray:
        """
        Winsorize 截断 + 均值归一化。

        截断原因：极端低波动期 σ̂ 接近 0，1/σ 爆炸至均值数百倍，
                 会使少数几天主导整个回归，引入新的不稳定性。
        """
        if self.winsorize:
            lo = np.percentile(w, self.winsorize_bounds[0])
            hi = np.percentile(w, self.winsorize_bounds[1])
            w  = np.clip(w, lo, hi)
        return w / w.mean()
    ...
    def _weights_park(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        """
        log(ε²) = Xγ + v  →  OLS 估计 γ  →  σ̂ = √exp(Xγ)  →  w = 1/σ̂

        量纲：1/σ（与 rolling / ewma 统一）
        前视偏差：Park 本身是截面方法，用全样本 OLS 残差在此场景下可接受；
                  若需严格无偏可替换为 _expanding_resid()。
        """
        # Park 辅助回归仍用全样本 OLS（截面假设，时序场景可选换）
        ...
        # σ̂ = √exp(Xγ)，w = 1/σ̂（量纲统一：标准差倒数）
        sigma_hat = np.sqrt(np.exp(Xc @ beta) + 1e-6)
        w = 1.0 / (sigma_hat + 1e-8)
    ...
    def _weights_rolling(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        """
        用 expanding OLS 残差做滚动标准差估计局部波动率。
        w = 1/σ_rolling

        改进（相对原版）：
          - 残差改用 expanding OLS，消除前视偏差
          - 量纲已为 1/σ，与原版一致，无需修改
          - 加入 winsorize 截断
        """
        resid = self._expanding_resid(X, y)       # ← 前视偏差修复

        s = pd.Series(resid)
        sigma = (s.rolling(self.rolling_window,
                           min_periods=max(5, self.rolling_window // 4))
                  .std()
                  .fillna(s.expanding(min_periods=5).std())
                  .fillna(s.std())
                  .values)
    ...
    def _weights_ewma(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        """
        用 expanding OLS 残差做 EWMA 标准差估计。
        w = 1/σ_ewma

        改进（相对原版）：
          - 残差改用 expanding OLS，消除前视偏差
          - 量纲从 1/σ²（原版）统一为 1/σ，减少估计噪声的平方放大效应
          - 加入 winsorize 截断
        """
        resid    = self._expanding_resid(X, y)     # ← 前视偏差修复
        ...
        sigma_est = np.sqrt(var_est)               # ← 量纲统一：取根号
        w = 1.0 / (sigma_est + 1e-8)              # 1/σ（原版为 1/σ²）
        return self._postprocess(w)
    ...
    def print_coef(self):
        ...
        for name, coef in sorted(zip(names, self.coef_),
...
# 训练入口
# ══════════════════════════════════════════════════════════════════
def _train_wls(X_train: pd.DataFrame,
               y_train: pd.Series,
    ...
    参数
    ----
    weight_method    : "park" | "rolling" | "ewma"
    rolling_window   : rolling 模式的窗口长度（默认 60）
    ewma_halflife    : ewma 模式的半衰期（默认 21）
    min_fit_samples  : expanding OLS 启动所需最少样本数（默认 30）
    winsorize        : 是否对权重做 Winsorize 截断（默认 True）
    winsorize_bounds : Winsorize 分位数上下界（默认 (1, 99)）
    """
    scaler = None
    X = X_train.values
